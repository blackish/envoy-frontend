<template>
  <div>
    ...
    <el-row>
      <el-col :span="24">
        <el-input :value="ignoreHealthOnHostRemoval" :disabled="!ignoreHealthOnHostRemovalDisabled">
          <template slot="prepend">Ignore Health On Host Removal</template>
          <template slot="append"><el-button icon="el-icon-edit" @click="ignoreHealthOnHostRemovalVisible=true"></el-button></template>
        </el-input>
      </el-col>
    </el-row>
    <el-row>
    ...
    <el-dialog title="Ignore Health On Host Removal" :visible.sync="ignoreHealthOnHostRemovalVisible">
      <el-form ref="ignoreHealthOnHostRemovalForm">
        <el-form-item label="Enable ignore health on host removal">
          <el-checkbox v-model="ignoreHealthOnHostRemovalForm.enabled"></el-checkbox>
        </el-form-item>
        <div v-if="ignoreHealthOnHostRemovalForm.enabled">
          <el-form-item label="Ignore health on host removal">
            <el-checkbox v-model="ignoreHealthOnHostRemovalForm.ignoreHealthOnHostRemoval"></el-checkbox>
          </el-form-item>
        </div>
        <el-button type="primary" @click="submitIgnoreHealthOnHostRemoval">Set</el-button>
        <el-button>Cancel</el-button>
      </el-form>
    </el-dialog>
...
//        params: JSON.stringify(this.configCluster.data.clusters[0]),
  ...
  computed: {
    ...
    ignoreHealthOnHostRemoval: function() {
      if ( typeof this.configCluster.data.clusters[0].ignoreHealthOnHostRemoval != 'undefined' ){
        return this.configCluster.data.clusters[0].ignoreHealthOnHostRemoval
      }
      return ""
    },
    ...
    ignoreHealthOnHostRemovalDisabled: function() {
      if ( typeof this.configCluster.data.clusters[0].ignoreHealthOnHostRemoval == 'undefined' ){
        return false
      }
      return true
  ...
  methods: {
    submitIgnoreHealthOnHostRemoval: function() {
      if ( !this.ignoreHealthOnHostRemovalForm.enabled ) {
        if ( typeof this.configCluster.data.clusters[0].ignoreHealthOnHostRemoval != 'undefined' ){
          this.$delete(this.configCluster.data.clusters[0], "ignoreHealthOnHostRemoval")
        }
      } else {
        this.$set(this.configCluster.data.clusters[0],"ignoreHealthOnHostRemoval", this.ignoreHealthOnHostRemovalForm.ignoreHealthOnHostRemoval)
      }
      this.ignoreHealthOnHostRemovalVisible = false
    },
